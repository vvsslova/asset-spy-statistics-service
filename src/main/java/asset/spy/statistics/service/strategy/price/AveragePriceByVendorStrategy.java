...
public class AveragePriceByVendorStrategy implements StatisticsStrategy<Map<String, BigDecimal>> {
    ...
    public Map<String, BigDecimal> calculate() {
        try (Stream<ProductEntity> stream = dataProvider.getAllProductsAsStream()) {
            return priceCalculator.calculateAveragePrice(stream, p -> p.getVendor().getName());
        } catch (Exception e) {
            throw new RuntimeException("Error calculating average price by vendor", e);
        }
    }
}
