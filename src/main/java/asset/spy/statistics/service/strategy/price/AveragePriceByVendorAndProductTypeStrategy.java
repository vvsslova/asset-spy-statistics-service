...
public class AveragePriceByVendorAndProductTypeStrategy implements StatisticsStrategy<Map<String, BigDecimal>> {
    ...
    public Map<String, BigDecimal> calculate() {
        try (Stream<ProductEntity> stream = dataProvider.getAllProductsAsStream()) {
            return priceCalculator.calculateAveragePrice(
                    stream,
                    p -> p.getVendor().getName() + " - " + p.getType()
            );
        } catch (Exception e) {
            throw new RuntimeException("Error calculating average price by vendor and product type", e);
        }
    }
}
