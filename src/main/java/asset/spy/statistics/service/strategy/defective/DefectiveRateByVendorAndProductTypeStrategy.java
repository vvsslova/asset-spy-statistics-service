package asset.spy.statistics.service.strategy.defective;
...
public class DefectiveRateByVendorAndProductTypeStrategy implements StatisticsStrategy<Map<String, Double>> {
    ...
    public Map<String, Double> calculate() {
        try (Stream<ProductItemStatusEntity> stream = dataProvider.getAllStatusesAsStream()) {
            return calculator.calculateRate(
                    stream,
                    s -> ProductItemStatus.DEFECTIVE == ProductItemStatus.fromString(s.getStatus()),
                    s -> s.getProductItem().getProduct().getVendor().getName() +
                            " - " + s.getProductItem().getProduct().getType()
            );
        } catch (Exception e) {
            throw new RuntimeException("Error calculating defect rate by vendor and product type", e);
        }
    }
}
