...
public class LostRateByVendorAndProductTypeStrategy implements StatisticsStrategy<Map<String, Double>> {
    ...
    public Map<String, Double> calculate() {
        try (Stream<ProductItemStatusEntity> stream = dataProvider.getAllStatusesAsStream()) {
            return calculator.calculateRate(
                    ...
                    s -> s.getProductItem().getProduct().getVendor().getName() + " - " + s.getProductItem().getProduct().getType()
            );
        } catch (Exception e) {
            throw new RuntimeException("Error calculating lost rate by vendor and product type", e);
        }
    }
}
