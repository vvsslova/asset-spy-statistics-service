...
public class AverageStatusDurationStrategy implements StatisticsStrategy<Map<String, Duration>> {
    ...
    public Map<String, Duration> calculate() {
        try (Stream<ProductItemStatusEntity> stream = dataProvider.getAllStatusesAsStream()) {
            return calculator.calculateStatusDurations(stream, s -> "").getOrDefault("", Map.of());
        } catch (Exception e) {
            throw new RuntimeException("Error calculating average status duration for all statuses", e);
        }
    }
}
