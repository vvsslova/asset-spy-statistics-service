...
public class AverageStatusDurationByTypeStrategy implements StatisticsStrategy<Map<String, Map<String, Duration>>> {
    ...
    public Map<String, Map<String, Duration>> calculate() {
        try (Stream<ProductItemStatusEntity> stream = dataProvider.getAllStatusesAsStream()) {
            return calculator.calculateStatusDurations(stream, s -> s.getProductItem().getProduct().getType());
        } catch (Exception e) {
            throw new RuntimeException("Error calculating average status duration by product type", e);
        }
    }
}
